...
class General {
	...
	public function scripts() {
		...
		wp_enqueue_style( 'ecomus-woocommerce-style', apply_filters( 'ecomus_get_style_directory_uri', get_template_directory_uri() ) . '/' . $style_file, array(),  '20240809' );
	...
	public function cart_subtotal( $cart_subtotal, $compound, $cart ) {
		...
		if ( WC()->cart->display_prices_including_tax()  ) {
			$total_regular_price = floatval( wc_get_price_including_tax( $product, array( 'price' => $total_regular_price ) ) );
			$total_sale_price =  wc_get_price_including_tax( $product, array( 'price' => WC()->cart->get_subtotal() ) );

			if( floatval( wc_get_price_including_tax( $product, array( 'price' => WC()->cart->get_subtotal() ) ) ) > 0 && floatval( $total_regular_price ) > floatval( wc_get_price_including_tax( $product, array( 'price' => WC()->cart->get_subtotal() ) ) ) ) {
