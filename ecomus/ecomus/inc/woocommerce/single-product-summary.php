...
class Single_Product_Summary extends Product_Base {
	...
	public function __construct() {
		...
		if ( intval( \Ecomus\Helper::get_option( 'product_ask_question' ) ) && ! empty( \Ecomus\Helper::get_option( 'product_ask_question_form' ) ) ) {
			add_action( 'ecomus_woocommerce_single_product_summary', array( \Ecomus\WooCommerce\Single_Product\Ask_Question::instance(), 'ask_question' ), 47 );
		}

		if ( intval( \Ecomus\Helper::get_option( 'product_delivery_return' ) ) && ! empty( \Ecomus\Helper::get_option( 'product_delivery_return_page' ) ) ) {
			add_action( 'ecomus_woocommerce_single_product_summary', array( \Ecomus\WooCommerce\Single_Product\Delivery_Return::instance(), 'delivery_return' ), 47 );
		}

		if ( intval( \Ecomus\Helper::get_option( 'product_share' ) ) ) {
			add_action( 'ecomus_woocommerce_single_product_summary', array( \Ecomus\WooCommerce\Single_Product\Share::instance(), 'product_share' ), 47 );
		}
