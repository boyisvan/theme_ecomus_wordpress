...
class Product_Attribute {
		...
		switch ( \Ecomus\WooCommerce\Helper::get_product_card_layout() ) {
			...
			case '8':
				...
				if( ! empty( Helper::get_option('product_card_attribute') ) ) {
					add_action( 'woocommerce_after_shop_loop_item', array( $this, 'product_attribute' ), 10 );
				}
			...
			case 'list':
				if( ! empty( Helper::get_option('product_card_attribute_second') ) ) {
					add_action( 'woocommerce_after_shop_loop_item', array( $this, 'product_attribute_second' ), 15 );
				}
				if( ! empty( Helper::get_option('product_card_attribute') ) ) {
					add_action( 'woocommerce_after_shop_loop_item', array( $this, 'product_attribute' ), 10 );
				}
	...
	public function product_attribute( $product = null ) {
		...
		if( $output ) {
			echo sprintf('<div class="product-variation-items em-flex em-flex-align-center">%s</div>', $output);
		}
	}
	...
	public function product_attribute_second( $product = null, $type = null ) {
		...
		$attribute_taxonomy = empty( $attribute_taxonomy ) ? 'pa_' . sanitize_title( self::$product_attribute_second ) : $attribute_taxonomy;
		if ( $attribute_taxonomy == 'none' ) {
			return;
		}
		...
		if( $product->get_type() == 'variable' ) {
			...
			if( $type == 'number' ) {
				$output = sprintf( // WPCS: XSS OK.
					esc_html( _nx( '%1$s size available', '%1$s sizes available', 'sizes available', count( $available_attributes ), 'ecomus' ) ),
			...
			if ( ! $product_attribute['is_taxonomy'] ) {
				...
				if( $post_terms ) {
					...
					if( $type == 'number' ) {
						$output = sprintf( // WPCS: XSS OK.
							esc_html( _nx( '%1$s size available', '%1$s sizes available', 'sizes available', count( $post_terms ), 'ecomus' ) ),
	...
	public function swatch_html( $swatches_args, $variation_args ) {
		...
		switch ( $type ) {
			case 'color':
				$html = sprintf(
					'<span class="product-variation-item product-variation-item--color em-tooltip %s" %s data-tooltip="%s" data-tooltip_position="bottom"><span class="product-variation-item__color" style="background-color:%s;"></span></span>',
					esc_attr( $css_class ),
					$variation_attrs,
			...
			case 'image':
				if ( $swatches_value ) {
					...
					$html  = sprintf(
						'<span class="product-variation-item product-variation-item--image em-tooltip %s" %s data-tooltip="%s" data-tooltip_position="bottom"><span>%s</span></span>',
						esc_attr( $css_class ),
						$variation_attrs,
						esc_attr( $attribute_name ),
						$image
			...
			default:
				...
				$html  = sprintf(
					'<span class="product-variation-item product-variation-item--label em-tooltip %s" %s data-tooltip="%s" data-tooltip_position="bottom">%s</span>',
					esc_attr( $css_class ),
					$variation_attrs,
					esc_attr( $attribute_name ),
					esc_html( $label )
