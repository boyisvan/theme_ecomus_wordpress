...
class Base {
	...
	public function loop_start( $html ) {
		...
		if ( intval( Helper::get_option( 'mobile_product_card_atc' ) ) && ! in_array( $loop_layout, array( '6', '7' ) ) ) {
	...
	public function product_featured_icons_open() {
		...
		if( \Ecomus\WooCommerce\Helper::get_product_card_layout() == '6' ) {
	...
	public function product_add_to_cart_link( $html, $product, $args ) {
		...
		if( isset( $args['button_classes'] ) ) {
			...
			$button_classes =  $product_card == '7' || $product_card == '4' ? 'em-button' : $button_classes;
        ...
        echo sprintf(
            ...
            Icon::inline_svg( 'icon=shopping-bag' ),
            esc_html( $product->add_to_cart_text() )
        );
    }
	...
	public function product_card_title() {
		echo '<h2 class="woocommerce-loop-product__title em-font-normal">';
			$this->loop_product_link_open();
				the_title();
			$this->loop_product_link_close();
		echo '</h2>';
